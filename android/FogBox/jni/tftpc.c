...
//char path[128] = "/mnt/sdcard/DCIM/Camera/";
char filepath[128];
int port = 69;
...
//add for android use CMY 2015/05/21
JNIEnv *jenv;
jobject jobj;
...
JNIEXPORT void JNICALL Java_com_android_fogbox_NativeLib_sendFile
  (JNIEnv * env, jobject object,jstring string1, jstring string2, jstring string3,jstring string4)
{
	jenv = env;
	jobj = object;
	//mgr =assetmanager;
	...
	const char *str4 = (*env)->GetStringUTFChars(env, string4, 0);
	
	__android_log_write(ANDROID_LOG_INFO, "tftp-client", "sendfile entered");
	__android_log_write(ANDROID_LOG_INFO, "tftp-client", str1);
	__android_log_write(ANDROID_LOG_INFO, "tftp-client", str2);
	__android_log_write(ANDROID_LOG_INFO, "tftp-client", str3);
	__android_log_write(ANDROID_LOG_INFO, "tftp-client", str4);

    sendRequestedFile(str1,str2,str3,str4);
...
sendRequestedFile(char* serverIP,char* portNo, char* fpath,char* fName)
{
  /*local variables */
  extern char *optarg;
  int sock, server_len, len, opt;	//,n;
...
//      return 0;
//    }
  if (!(host = gethostbyname (serverIP)))
    {
      perror ("Client could not get host address information");
      exit (2);
    }
...
//	case 'p':		/* put a file on the server */
	  ...
	  __android_log_write(ANDROID_LOG_INFO, "tftp-client", filepath);
	  opcode = WRQ;
	  fp = fopen (filepath, "r");	/*opened the file for reading */
	  if (fp == NULL)
	    {
	      printf ("Client: file could not be opened\n");
	      __android_log_write(ANDROID_LOG_INFO, "tftp-client", "Client: file could not be opened");
	      return 0;
	    }
//	  if (debug)
//	    {
...
//		      filename);
//	    }
	  __android_log_write(ANDROID_LOG_INFO, "tftp-client", "Client: file opened");
	  fclose (fp);
//	  break;
//	case 'g':		/*get a file from the server */
//	  strncpy (filename, optarg, sizeof (filename) - 1);
...
/* Done dealing with switches and the command line*/
  ...
  if ((sock = socket (AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0)
    {
      printf ("Client: Socket could not be created");
      __android_log_write(ANDROID_LOG_INFO, "tftp-client", "Client:Socket could not be created");
      return 0;
    }

  __android_log_write(ANDROID_LOG_INFO, "tftp-client", "Client:Socket created");

/*set the address values for the server */
  memset (&server, 0, sizeof (server));	/*Clear the structure */
  __android_log_write(ANDROID_LOG_INFO, "tftp-client", "memset server");
  server.sin_family = AF_INET;	/*address family for TCP and UDP */
  memcpy (&server.sin_addr, host->h_addr, host->h_length);	/*set address of server taken from gethostbyname function */
  //server.sin_addr.s_addr = htonl (INADDR_ANY); /*use any address */
  server.sin_port = htons (port);	/*pick a free port */
...
//  if (debug)
//    printf ("Client: size of server_address is : %d bytes\n", server_len);
  __android_log_write(ANDROID_LOG_INFO, "tftp-client", "Before clear buf");
  memset (buf, 0, BUFSIZ);	/*clear the buffer */
  /* this is the first request message */
  len = req_packet (opcode, filename, mode, buf);

  __android_log_write(ANDROID_LOG_INFO, "tftp-client", "requestPkt");
...
//      break;
    case WRQ:			/*write to the server, upload */
      tsend (filepath, server, mode, sock);
      break;
    default:
      printf ("Invalid opcode detected. Ignoring packet.");
    }
