...
int debug = 0;
char path[64] = "/root/rsync/media/USER1/tmp/";
char fog_realpath[64] = "/root/rsync/media/USER1/";
int port = 69;
...
tget (char *pFilename, struct sockaddr_in client, char *pMode, int tid)
{
  ...
  strncat (fullpath, filename, sizeof (filename));	//build the full file path by appending filename to path
...
/* sending error message */
  ...
  //Write tmp file to real file
	tcopytmp(filename, sizeof(filename));


  return;
}

void tcopytmp(char *fileName, size_t len){
	int inputFd;
    int outputFd;	
	...
	char mytmpbuf[buf_size];
	char tmpfullpath[196], realtmpfullpath[196], realfullpath[196];
	char newname[128];
	char tmpnewname[128];
	char hostname[128];
	...
	strcpy(tmpnewname,newname);
	strncat(tmpnewname,".tmp",4);

	strcpy(tmpfullpath,path);
  	strncat (tmpfullpath, fileName, len);	//build the full file path by appending filename to path
  	
	strcpy(realtmpfullpath,fog_realpath);
	strncat(realtmpfullpath,tmpnewname,sizeof(tmpnewname));

	strcpy(realfullpath,fog_realpath);
	strncat(realfullpath,newname,sizeof(newname));


	if (debug)
			printf("fileName:%s, hostname:%s, newname:%s,tmpnewname:%s,tmpfullpath:%s,realtmpfullpath:%s,realfullpath:%s\n",fileName,hostname,newname,tmpnewname,tmpfullpath,realtmpfullpath,realfullpath);

	
	/* open input and outpu files */
    inputFd = open(tmpfullpath, O_RDONLY);
    if (inputFd == -1){
		if (debug)
			printf("cannot open input tmp file: %s\n",tmpfullpath);
		return;
	}
    ...
    outputFd = open(realtmpfullpath, openFlags, filePerms);
	...
	while ((numRead = read(inputFd, mytmpbuf, buf_size)) > 0)
	{
    	if (write(outputFd, mytmpbuf, numRead) != numRead){
			if (debug)
	...
	rename(realtmpfullpath, realfullpath);
}
