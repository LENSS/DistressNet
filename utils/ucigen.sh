...
/sbin/uci set network.wifi.ipaddr=192.168.50.$1
/sbin/uci set network.wifi.netmask=255.255.255.0

"
if [ $1 -eq 1 ]
then
echo "/sbin/uci set network.lan.gateway=192.168.1.100"
else
echo "/sbin/uci set network.wifi.gateway=192.168.50.1"
fi
...
/sbin/uci set wireless.radio0.txpower=25
/sbin/uci set wireless.radio1.txpower=17

/sbin/uci set wireless.radio0.htmode=HT40+
/sbin/uci set wireless.radio0.noscan=1
...
/sbin/uci set wireless.@wifi-iface[1].ssid=324-cjh-adhoc5
...
/sbin/uci set fstab.@mount[0].target=/mnt/sandisk
